...
function LoginPageInner() {
  ...
  if (typeof window !== "undefined" && searchParams.get("next")) {
    // force re-render with next path — noop
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 relative overflow-hidden"
      ...
      >
        ...
        >
          <motion.div
            className="inline-flex h-14 w-14 rounded-2xl bg-white/10 backdrop-blur-sm items-center justify-center mb-4 ring-1 ring-white/20"
        ...
        <motion.div
          className="rounded-2xl p-8 sm:p-10 backdrop-blur-sm"
          style={{
        ...
        >
            ...
            <div className="flex bg-white/5 rounded-xl p-1 mb-6">
              <button type="button" onClick={() => { setMode("login"); setError(null); }}
                className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${
                  mode === "login" ? "bg-white/10 text-white shadow-sm" : "text-white/50 hover:text-white/80"
                }`}>Sign In</button>
              <button type="button" onClick={() => { setMode("register"); setError(null); }}
                className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${
                  mode === "register" ? "bg-white/10 text-white shadow-sm" : "text-white/50 hover:text-white/80"
                }`}>Register</button>
            </div>
          </motion.div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              {mode === "register" && (
                <div>
                  ...
                  <input type="text" placeholder="Rohan Sharma" value={name}
              ...
              <div>
                ...
                <input type="email" placeholder="rohan@example.com" value={email}
                  onChange={(e) => setEmail(e.target.value)} required autoComplete="email"
                  className="w-full px-4 py-3 rounded-xl text-sm bg-white/5 border border-white/10 text-white placeholder-white/30 outline-none focus:border-white/30 focus:bg-white/10 transition-all duration-200" />
              </div>
              <div>
                <label className="block text-sm font-medium text-white/80 mb-1.5">Password</label>
                <input type="password" placeholder={mode === "register" ? "Min. 8 characters" : "••••••••"} value={password}
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.02, boxShadow: "0 4px 20px rgba(3,105,161,0.3)" }}
                whileTap={{ scale: 0.98 }}
              >
          ...
          {mode === "login" && (
            <motion.div variants={slideUpFade} className="mt-6">
                ...
                <div className="space-y-1.5">
                  {[
                    ...
                    >
                      ...
                      <span className="flex-1 text-white/80 font-medium min-w-0">{d.email}</span>
          ...
          <motion.p variants={slideUpFade} className="text-center text-sm text-white/50 mt-6">
            {mode === "login" ? (
              <>No account?{" "}
                <button type="button" onClick={() => { setMode("register"); setError(null); }}
                  className="text-white/80 hover:text-white font-medium hover:underline transition-colors">Create one</button>
              </>
            ) : (
              <>Already registered?{" "}
                <button type="button" onClick={() => { setMode("login"); setError(null); }}
                  className="text-white/80 hover:text-white font-medium hover:underline transition-colors">Sign in</button>
              </>
            )}
          </motion.p>

          {mode === "login" && (
            <motion.div variants={slideUpFade} className="text-center mt-3">
