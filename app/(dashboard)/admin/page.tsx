...
export default function AdminPage() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const url = editing ? "/api/admin/questions/update" : "/api/admin/questions/create";
      const method = editing ? "PUT" : "POST";
      const body = editing ? { id: editing.id, ...draft } : draft;
      cli.api(method, url, body);
      ...
      cli.res(method, url, res.status, data);
      if (!res.ok) {
        alert(data.error || "Save failed");
        return;
      }
      cli.success(`Question ${editing ? "updated" : "created"}: id=${data.id}`);
      setEditing(null);
  ...
  const remove = async (id: number) => {
    ...
    cli.api("DELETE", `/api/admin/questions/delete?id=${id}`);
    try {
      const res = await fetch(`/api/admin/questions/delete?id=${id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
        alert(data.error || "Delete failed");
        return;
      }
      cli.success(`Deleted question ${id}`);
      loadQuestions(filterSetId);
    } catch (e) {
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      const data = await res.json();
      cli.res("POST", "/api/upload", res.status, data);
      if (!res.ok) {
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", padding: "48px 56px 96px" }}>
          ...
          <div className="grid grid-cols-2 gap-5 mb-7">
            ...
            <div>
              ...
              <select
                value={draft.type}
                onChange={(e) => setDraft({ ...draft, type: e.target.value as "mcq" | "numeric" })}
                style={inputStyle}
              >
                ...
                {["A", "B", "C", "D"].map((letter, i) => {
                  ...
                  return (
                    <div key={letter} className="flex gap-3 items-center">
                      ...
                      />
                      <button
                        onClick={() => setDraft({ ...draft, correctAnswer: letter })}
                        className="shrink-0"
                        style={{
                          padding: "8px 14px",
                          borderRadius: 8,
